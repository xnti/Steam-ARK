...
ImFont* smallest_pixel, * SpecialFont;
...
auto TorporBar(float x, float y, float w, float h, int ptorpor, bool Outlined)-> VOID
{
	auto vList = ImGui::GetOverlayDrawList();
	...
	RectFilled(x, y, x + w, y + (int)(((float)h / 100.0f) * (float)ptorpor), barColor, 0.0f, 0);
}
...
auto Button1(const char* label, int* index, int val, const ImVec2& size_arg, bool sameline) -> VOID
{
	if (*index == val)
	{
